    ...
    <Container variant="small" className="setup-container">
      ...
      <section className="tour-players">
        {tournament.status !== 'finished' ? (
          players.map((player, idx) => <DraggablePlayer key={idx} player={player} />)
        ) : (
          players.map((player, idx) => (
            <DraggablePlayer
              key={idx}
              player={player}
              won={player.username === tournament.winner ? true : false}
            />
          ))
        )}
        ...
        <Text variant="purple">
          {players.length === tournament.max
            ? tournament.status === 'finished'
              ? null
              : 'Players ready!'
            : `There's not enough people to start yet!`}
        </Text>
      </section>
    </Container>
  );
...
const DraggablePlayer: React.FC<{ player: UserState; won?: boolean }> = ({ player, won }) => {
  ...
  return (
    <div className={`tour-profile ${won ? 'winner' : ''}`} ref={dragRef}>
